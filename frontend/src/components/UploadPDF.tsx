...
const UploadPDF = () => {
    ...
    const handleUpload = async () => {
        ...
        formData.append("pdf", file); // "pdf" must match multer's `.single('pdf')`

        try {
            const uploadPdf = await axios.post(
                "http://localhost:3000/ingest-pdf",
                formData,
                {
                    headers: {
                        "Content-Type": "multipart/form-data", // ✅ This is OK with axios
                    },
                }
            );

            navigate(`/chat/${uploadPdf.data.collectionName}`);
        } catch (err) {
